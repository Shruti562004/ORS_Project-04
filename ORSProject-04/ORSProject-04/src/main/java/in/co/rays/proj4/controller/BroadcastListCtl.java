...
public class BroadcastListCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			List<BroadcastBean> list = model.search(bean, pageNo, pageSize);
			List<BroadcastBean> next = model.search(bean, pageNo + 1, pageSize);
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					BroadcastBean deletebean = new BroadcastBean();
					for (String id : ids) {
